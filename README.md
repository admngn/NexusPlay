...
```
   ...
   reverse proxy           reverse proxy (redondance)
        │                         │
        └────────────┬────────────┘
                     │
       ┌─────────────┴─────────────┐
...
| URL | Service |
|---|---|
| http://3.84.37.64/ | Frontend (via nginx1) |
| http://3.84.37.64/api/hello | Backend (via nginx1) |
| http://3.84.37.64/health | Healthcheck nginx |
| http://13.221.93.203/ | Frontend (via nginx2) |
| http://3.84.37.64:3001 | Grafana (`admin` / `admin`) |
| http://3.84.37.64:9090 | Prometheus |
| http://3.84.37.64:9093 | Alertmanager |
...
| 2 | Équilibrage de charge avec redondance | 2× nginx (`nginx`/`nginx2`) + upstream `least_conn` sur 3-5 backends | 🟡 (DNS round-robin BIND9 prévu) |
...
```bash
for i in 1 2 3 4 5 6; do curl -s http://3.84.37.64/api/hello | python3 -c "import sys,json; print(json.load(sys.stdin)['hostname'])"; done
```

**Autoscaling** — lance le load test, regarde les logs en parallèle :
```bash
# Terminal 1
k6 run --env BASE_URL=http://3.84.37.64 k6/load.js
